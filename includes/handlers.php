...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'savetemplate') {
        try {
            ...
            auditLog('template_saved', ['template_name' => $templateName, 'collection' => $collectionName], 'info', 'data');
    ...
    if ($action === 'deletetemplate') {
        try {
            ...
            auditLog('template_deleted', ['template_name' => $templateName, 'collection' => $collectionName], 'info', 'data');
    ...
    if ($action === 'fieldstats') {
        try {
            ...
            auditLog('field_statistics_generated', ['field' => $fieldName, 'unique_values' => count($statsArray)], 'info', 'data');
    ...
    if ($action === 'bulkupdate') {
        if (!userHasPermission('bulk_operations')) {
            $message = '❌ Permission denied: You do not have permission to perform bulk operations';
            $messageType = 'error';
            auditLog('permission_denied', ['action' => 'bulkupdate'], 'warning', 'security');
        } else {
            try {
                ...
                auditLog('bulk_update_match', ['match_field' => $matchField, 'update_field' => $updateField, 'count' => $result->getModifiedCount()], 'info', 'data');
    ...
    if ($action === 'findreplace') {
        if (!userHasPermission('edit_data')) {
            $message = '❌ Permission denied: You do not have permission to modify data';
            $messageType = 'error';
            auditLog('permission_denied', ['action' => 'findreplace'], 'warning', 'security');
        } else {
            try {
                ...
                auditLog('find_replace', ['field' => $fieldName, 'find' => $findValue, 'replace' => $replaceValue, 'count' => $result->getModifiedCount()], 'info', 'data');
    ...
    if ($action === 'add') {
        if (!userHasPermission('create_data')) {
            ...
            $messageType = 'error';
            auditLog('permission_denied', ['action' => 'add'], 'warning', 'security');
        } else {
            try {
                ...
                auditLog('document_added_handler', ['fields_count' => count($data)], 'info', 'data');
